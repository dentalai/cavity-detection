...
class GUI():
    ...
    def common_config(self, title='Dashboard '): #(Beta version :golf:)
        ...
        """
        st.image("./media/logo_inveesion.png","InVision.", width=50)
        ...
        self.dataSource = st.sidebar.radio(
                'Please select the source of your ' + self.appType, ['Image: Upload'])
    ...
    def appDescription(self):
        ...
        if self.selectedApp == 'Object Detection':
            st.info(
                'This application performs dental decay/cavity detection using advanced artificial intellgence system.')
            ...
            st.info(
                'To start using InVision dashboard you must first select an Application from the sidebar menu other than Empty')
